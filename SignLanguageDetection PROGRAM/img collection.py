...
# Preview display size (the saved images remain the cropped frame size)
DISPLAY_WIDTH = 960
DISPLAY_HEIGHT = 720
...
for j in range(number_of_classes):
    ...
    while not done:
        ...
        # Prepare a larger preview for display so overlay text isn't cut off
        try:
            display_frame = cv2.resize(frame, (DISPLAY_WIDTH, DISPLAY_HEIGHT))
        except Exception:
            display_frame = frame

        # Display a message on the preview to indicate readiness (asking the user to press "Q")
        cv2.putText(display_frame, 'Ready? Press "Q" ! :)', (50, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.1, (0, 255, 0), 2, cv2.LINE_AA)
        # Show the preview frame in a window titled 'frame'
    ...
    while counter < dataset_size:  # Continue capturing until the dataset size is reached
        ...
        # Show the current frame in the window again (scaled for preview)
        try:
            display_frame = cv2.resize(frame, (DISPLAY_WIDTH, DISPLAY_HEIGHT))
        except Exception:
            display_frame = frame
